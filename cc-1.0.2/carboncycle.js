...
// conversion factor to go from PgC to PPM CO2
var convertPgCtoPPM =
    (1e15			// Peta-
     /
     12) *			// mC (molecular weight of Carbon)
    (28.97			// mATM (avg molecular weight of the atmosphere
     /
     5.1e21) *			// MassATM (mass of the atmosphere)
    1e6;			// million, the M in `PPM'
...
function init () {
  ...
  plot[0].axis = {
    ...
    y1: { range: [-10, 35],
	  ...
	  units: 'PgC / year',
	  data: [ [], [], [], [] ],
	  spread: [ [  -2.1, 32.0 ],	// estimated spread at year 2100
		    [  -2.1,  0.4 ],
		    [ -10.0, -3.8 ],
		    [ -10.0,  6.0 ]
		  ],
  ...
  plot[1].axis = {
    ...
    y1: { range: [300, 1200],
	  ...
	  units: 'ppm',
	  data: [ [] ],
	  spread: [ [ 433, 1133 ] ],	// estimated spread at year 2100
	  proj: [ [], [], [], [] ],	// keep previous
	  active: 0		// only one
	},
    y2: { range: [-1, 6],		// calculated later on
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    // keep track of atmospheric CO2 for 1986-2005 to calculate mean
    if ((data[i][0] >= 1986) && (data[i][0] <= 2005)) {
      total += data[i][6];
      count++;
    }
  }

  // use the mean value of CO2 ppm as the basis for the temperature scales
  plot[1].coord.temp.y0 = total / count;
...
function draw_segment (curr) {
  ...
  if (prev > 0) {
    ...
    plot[0].ctx.arc (pt[prev].canv[0], pt[prev].canv[1], pt_size,
		 0, 2 * Math.PI, true);
    plot[0].ctx.fillStyle = ((mouse.follow || touch.follow) &&
			     (prev == plot[0].axis.y1.selected)) ?
